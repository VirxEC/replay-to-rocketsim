...
/// Map Rocket League body product ids from replay loadouts to `RocketSim` hitbox families.
///
/// Product ids can be resolved to item names with `RLBot`'s `items.csv`, but hitbox family is not
/// present in that CSV. Keep this table conservative and fall back when unknown.
pub(crate) fn car_body_config_for_product_id(product_id: u32) -> Option<CarBodyConfig> {
    match product_id {
        // Octane hitbox family.
...
mod tests {
    ...
    #[test]
    fn maps_common_body_product_ids_to_rocketsim_hitboxes() {
        assert_eq!(
            car_body_config_for_product_id(23),
            Some(CarBodyConfig::OCTANE)
        );
        assert_eq!(
            car_body_config_for_product_id(4284),
            Some(CarBodyConfig::OCTANE)
        );
        assert_eq!(
